...
class FPN(nn.Module):
    def __init__(self):
        ...
        self.toplayer1 = MRF(256)
        self.toplayer2 = MRF(256)
        self.toplayer3 = MRF(256)
...
class MRF2(nn.Module):
    def __init__(self,in_planes):
        super(MRF2,self).__init__()
        self.share_weight=nn.Parameter(torch.randn(in_planes,in_planes,3,3))
        self.bn1=nn.BatchNorm2d(in_planes)
        self.bn2=nn.BatchNorm2d(in_planes)
        self.bn3=nn.BatchNorm2d(in_planes)
    ...
    def forward(self,x):
        #residual=x
        x1=F.conv2d(x,self.share_weight, stride=2, padding=1,bias=None, dilation=1)
        x1=self.bn1(x1)
        #x1=x1+residual
        x1=self.relu1(x1)
        x2=F.conv2d(x,self.share_weight, stride=2, padding=3,bias=None, dilation=3)
        x2=self.bn2(x2)
        #x2=x2+residual
        x2=self.relu2(x2)
        x3=F.conv2d(x,self.share_weight,stride=2, padding=5,bias=None, dilation=5)
        x3=self.bn3(x3)
        #x3=x3+residual
        x3=self.relu3(x3)
        y=torch.cat((x1, x2, x3), dim=1)
...
class S3FD(nn.Module):
    ...
    def __init__(self, phase, base_model, head, num_classes=2):
        ...
        self.layer6 = MRF2(256)                                      
        self.layer7 = MRF2(256)
        self.fpn=FPN()
        self.MR1=MRF(256)
        self.MR2=MRF(256)
        self.MR3=MRF(256)
